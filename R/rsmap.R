...
#' enz1 <- 'enz1'
#' enz2 <- 'enz2'
#' rsmap(enz1, frag1, enz2, frag2, bi_dig)
#'
#' @export
#' @import ggplot2
#' @import stringr
#'
rsmap <- function(enz1, frag1, enz2, frag2, dou_dig, name = 'Test Seq') {
  ...
  if ( len1 != len2 ) stop('The sum length of frag1 must be same with the sum length of frag2')
  ...
  d1 <- data.frame(xval=frag1, yval=5, Restriction_Enzymes = enz1, name = name)
  d2 <- data_frame(xval=frag2, yval=5, Restriction_Enzymes = enz2, name = name)
  dat <- rbind(d1, d2)

  print('Solution Found--------------------')
  print('Plotting--------------------------')

  title <- str_c('Restriction Site Map for ', name, ' (length = ', len1, ')')

  ggplot(dat, aes(x=xval, y=yval, color=Restriction_Enzymes, shape=Restriction_Enzymes)) + xlim(0, len1) + geom_point(size=3)  + geom_hline(yintercept = 5)+
       theme_minimal() +
           theme(
  ...
  cat(nrow(dat), 'Restriction Site(s) on', name, ':', '\n______', enz1, 'cut at', d1$xval, '\n______', enz2, 'cut at', d2$xval)
